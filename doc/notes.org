...
2. Replace the whole text in mkspecs/qws/linux-arm-g++/qmake.conf by the following:
   #+BEGIN_EXAMPLE
     ...
     # modifications to g++.conf
     QMAKE_CC                = arm-linux-gnueabihf-gcc -mcpu=cortex-a8 -mtune=cortex-a8 -O0 -mfpu=neon -mfloat-abi=softfp -lrt
     QMAKE_CXX               = arm-linux-gnueabihf-g++ -mcpu=cortex-a8 -mtune=cortex-a8 -O0 -mfpu=neon -mfloat-abi=softfp -lrt
     QMAKE_LINK              = arm-linux-gnueabihf-g++ -mcpu=cortex-a8 -mtune=cortex-a8 -O0 -mfpu=neon -mfloat-abi=softfp -lrt
     QMAKE_LINK_SHLIB        = arm-linux-gnueabihf-g++ -mcpu=cortex-a8 -mtune=cortex-a8 -O0 -mfpu=neon -mfloat-abi=softfp -lrt
...
3. Configure Qt
   #+BEGIN_SRC sh
     make clean; ./configure -embedded arm -prefix /usr/local/qt -little-endian -no-qt3support -no-cups -no-largefile -optimized-qmake -no-openssl -xplatform qws/linux-arm-g++ -nomake tools -nomake examples -nomake demos -qt-mouse-tslib -no-webkit
   #+END_SRC
   + Now, this configuration is what I needed. You can drop in or add whatever you want. Some handy options that you may (or may not require) are :
     + Enable touchscreen library support : -qt-mouse-tslib
     + Enable USB keyboard support : -qt-kbd-linuxinput
