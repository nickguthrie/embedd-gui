...
** Configure Cross-Compiling Toolchain
...
#+BEGIN_SRC sh
cd ~/git
git clone http://github.com/kergoth/tslib.git tslib/
cd tslib
./autogen.sh
...
** TODO Configure QT for Embedded Linux - Buildroot Version
*** Steps
**** DONE Install QT on Development Platform
http://suchakra.wordpress.com/2011/05/17/qt-on-mini2440/
#+BEGIN_SRC sh
sudo apt-get install qtcreator
#+END_SRC
**** TODO Install TSLIB on Development Platform
***** Steps
#+BEGIN_SRC sh
  cd ~/git
  git clone http://github.com/kergoth/tslib.git tslib/
...
#+END_SRC
***** Links
**** TODO Install qt-everywhere on Development Platform
1. Install Source
   #+BEGIN_SRC sh
     tar -xvf qt-everywhere-opensource-src-4.6.2.tar.gz /usr/local/qt
   #+END_SRC
2. Replace the whole text in mkspecs/qws/linux-arm-g++/qmake.conf by the following:
   #+BEGIN_EXAMPLE
     ...
     # modifications to g++.conf
     QMAKE_CC                = arm-unknown-linux-gnueabi-gcc -mcpu=cortex-a8 -mtune=cortex-a8 -O0 -mfpu=neon -mfloat-abi=softfp
     QMAKE_CXX               = arm-unknown-linux-gnueabi-g++ -mcpu=cortex-a8 -mtune=cortex-a8 -O0 -mfpu=neon -mfloat-abi=softfp
     QMAKE_LINK              = arm-unknown-linux-gnueabi-g++ -mcpu=cortex-a8 -mtune=cortex-a8 -O0 -mfpu=neon -mfloat-abi=softfp
     QMAKE_LINK_SHLIB        = arm-unknown-linux-gnueabi-g++ -mcpu=cortex-a8 -mtune=cortex-a8 -O0 -mfpu=neon -mfloat-abi=softfp
     ...
     QMAKE_RANLIB            = arm-unknown-linux-gnueabi-ranlib
...
3. Configure Qt
   #+BEGIN_SRC sh
     ./configure -embedded arm -nomake demos -nomake example -xplatform qws/linux-arm-g++ -prefix /usr/local/qt -little-endian -no-qt3support -no-cups -no-largefile -optimized-qmake -no-openssl -nomake tools -qt-mouse-tslib
   #+END_SRC
   + Now, this configuration is what I needed. You can drop in or add whatever you want. Some handy options that you may (or may not require) are :
     + Enable touchscreen library support : -qt-mouse-tslib
     + Enable USB keyboard support : -qt-kbd-linuxinput
