...
This documentation is written under the [[GNU%20Free%20Documentation%20License][GNU Free Documentation License]]. The SVN repository with the source code for this project can be found [[http://code.google.com/p/embedd-gui/][here]]. This documentation is viewable as an [[./index.html][html info file]], a [[./full.html][single html file]], a [[./index.pdf][pdf]], and in its original [[http://nickguthrie.com/embedd_gui/doc/index.org][org-mode]] format. This project would not be possible without funding from the [[http://rcos.rpi.edu/][Rensselaer Center for Open Source]].  The RCOS Observatory Project page can be found [[http://rcos.rpi.edu/projects/embedd-gui/][here]]. \\
\\
...
** License
#+BEGIN_EXAMPLE
  Copyright (c)  2013  Nicholas Guthrie.
...
+ [[http://youtu.be/g0xSY7x0GME][Video]]
+ [[./doc/OSSEG3.pdf][Presentation Slides]]
** Prototype Lightswitch GUI Videos
+ [[http://youtu.be/XWMzWnA78z0][Vital Vio Controller]]
+ [[http://youtu.be/5cpHjrTNl-s][RPI RCOS Embedd-GUI Controller]]
** A13 OlinuXino 4.3" Touchscreen Qt Embedded Demo
+ [[http://youtu.be/v-ndT9fh3e0][Video]]
...
1. Download A13-OlinuXino Debian Image ([[http://nickguthrie.com/embedd_gui/bin/Finalized/nickguthrie-a13-olinuxino-complete-v1.image.gz][here]]).
...
1. Set up the [[Install%20ARM%20Toolchain%20with%20Buildroot][Cross Compiling Toolchain]]
2. Compile [[Configure%20QT%20for%20Embedded%20Linux][TSLIB from Source on Development Platform]]
3. [[Configure%20QT%20for%20Embedded%20Linux][Compile QT from Source]]
...
** Compiling Programs
1. You must first build the makefile using the Qmake compiled in the last step, mine exists in /usr/local/qt/bin/qmake.  If I had a program created in qt_creator called my_program:
   #+BEGIN_SRC sh
     /usr/local/qt/bin/qmake my_program.pro
   #+END_SRC
   + only needs to be done once on the system
2. Afterwards, compile the program with:
   #+BEGIN_SRC sh
     make clean; make
   #+END_SRC
...
1. Set up SFTP
   ...
   2. Set the local directory (where the binary is to send)
      #+BEGIN_SRC sh
        lcd /home/slaye/Desktop/myprogram/
      #+END_SRC
   3. Set the directory where you want to send the program
      #+BEGIN_SRC sh
        cd ./Desktop
      #+END_SRC
2. Send the program to the Device
   #+BEGIN_SRC sh
     put myprogram
   #+END_SRC
** Running a Program
1. Plug in the Wireless, and USB Keyboard before booting
   + WARNING: not plugging in the keyboard may mess up the settings that tell TSLIB where to look for touchscreen input.  Currently a keyboard is always needed, which is why it is configured this way, this can be modified in the [[Configure%20tslib%20on%20Device][TSLIB configuration]] on the device for more permanent results.
2. Plug in Power
3. Login:
   | Username | root |
   | Password | password |
4. Navigate to the Directory
   ...
   #+END_SRC
5. Once you are in the dirtory with your program (say, my_program), run it with:
   #+BEGIN_SRC sh
     ./my_program -qws
   #+END_SRC
   + the "-qws" is the command to run the virtual screen
6. Killing the Program
   ...
   1. Type Ctrl + Alt + F2 to switch to the second terminal
   2. Login (Username: root, Password, password)
   3. Find your process name (aka the CMD)
      #+BEGIN_SRC sh
        ps -a
      #+END_SRC
   4. Kill the Process
   ...
   5. Check the process is dead
      ...
      + you can kill it with more power with:
        ...
        #+BEGIN_SRC sh
          kill -9 PID
        #+END_SRC
        + where you use the PID found with "ps -a"
   6. Switch back to your other terminal with Ctrl + Alt + F1
...
+ To publish, in Emacs open the embedd-gui/docs/notes.org file, and type: C-u C-c C-e E.
  | C-u | forces all to be published, since it won't recoginize some files have changed due to the way they are linked |
  | C-c C-e | is the same thing as M-x org-export                                                                          |
  | E   | the option to export all projects                                                                            |
...
*** Main.cpp
#+INCLUDE: "../src/vital_vio/vital_vio/main.cpp" src cpp
*** MainWindow.cpp
#+INCLUDE: "../src/vital_vio/vital_vio/mainwindow.cpp" src cpp
*** MainWindow.h
#+INCLUDE: "../src/vital_vio/vital_vio/mainwindow.h" src cpp
*** moc_mainwindow.cpp
#+INCLUDE: "../src/vital_vio/vital_vio/moc_mainwindow.cpp" src cpp
*** ui_mainwindow.h
#+INCLUDE: "../src/vital_vio/vital_vio/ui_mainwindow.h" src cpp
*** mainwindow.ui
#+INCLUDE: "../src/vital_vio/vital_vio/!mainwindow.ui" src txt
** Burgler
*** Source
**** MainWindow.cpp
#+INCLUDE: "../src/testing/Burgler/mainwindow.cpp" src cpp
**** MainWindow.h
#+INCLUDE: "../src/testing/Burgler/mainwindow.h" src cpp
**** moc_mainwindow.cpp
#+INCLUDE: "../src/testing/Burgler/moc_mainwindow.cpp" src cpp
**** ui_mainwindow.h
#+INCLUDE: "../src/testing/Burgler/ui_mainwindow.h" src cpp
**** mainwindow.ui
#+INCLUDE: "../src/testing/Burgler/mainwindow.ui" src txt
*** Notes
+ [[http://stackoverflow.com/questions/1675992/how-do-i-set-a-background-color-for-the-whole-window-of-a-qt-application][Change the background color of the qt application]]
...
#+INCLUDE: "./license_gpl.txt" src txt
