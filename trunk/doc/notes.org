...
+ This should print a list of all processes running dd, which, if you aren't running dd for anything else, should just bring up one process.  To see the status, do:
  #+BEGIN_SRC sh
    kill -INFO gpid  
  #+END_SRC
  + replacing gpid with the process ID you found from the previous step.  This output will actually print to the terminal which is running the dd process, and will show you how many bytes have been written so far.  Once it's finished, you should have a Beagleboard ready SD card.
